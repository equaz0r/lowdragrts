...
- ✅ **Sun glint — deliberately fake screen-space tracking, not physical reflection** (`TerrainMaterial.ts`, 11 Aug 2026 rewrite) — two real bugs got fixed along the way (`sunDirection` frozen at material creation, never updated; then `cameraDirection` being origin-relative not per-fragment — see the bullet below), but after both, the glint STILL didn't track the sun once the camera moved off-centre. Simon's hand-annotated screenshot (green pen = expected reflection path, yellow = actual) proved why: that's correct reflection-law behaviour for an off-axis camera against a flat-orbit sun, not a bug — the true `reflect()` path genuinely diverges from "under the sun" the closer to camera it gets. Simon chose to fake it rather than chase physically-correct reflection further. Ripped out the whole physically-based term — gone: `sunDirection` uniform, N·L, `reflect()`, view-dot (and their `ReflectionParameters` weights). Replaced with a screen-space trick in `calculateReflection()`: compare the direction from camera→fragment against camera→sun (`LightingSystem.getSunPosition()` — raw, NOT normalized, pushed live every frame in `TerrainGenerator.update()`, distinct from the existing normalized `getSunDirection()` which other code still uses); when the two align, this fragment IS the sun's screen position, so the hotspot sits there by construction, independent of true reflection geometry. A soft facing-sun gate stops back-facing cliffs glinting just because they're screen-aligned. New `ReflectionParameters`: `SUN_GLINT_WEIGHT` (1.1) / `SUN_GLINT_SHARPNESS` (100.0) / `SUN_FACING_GATE_POWER` (0.4) — first-pass numbers, not yet visually confirmed, expect live retuning. Typechecks + builds clean; check this first next session.
...
- **Sun glint is deliberately non-physical now** (see What's Working above) — no `sunDirection`/`reflect()`/N·L left in the shader, replaced by `sunWorldPosition` + camera→fragment vs camera→sun screen-space alignment. If a reflection complaint comes in, check which system it actually is first: hotspot in the wrong SCREEN POSITION → this glint code (`SUN_GLINT_*` params); too bright/dim/rare in general → the weighted-sum/clamp system below, which is unchanged and separate.
- `SUN_GLINT_WEIGHT` (1.1) / `SUN_GLINT_SHARPNESS` (100.0, higher = tighter hotspot) / `SUN_FACING_GATE_POWER` (0.4) — untested first-pass numbers, not data-driven like the weights below. Retune live if the hotspot is the wrong size/brightness.
