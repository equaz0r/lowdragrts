...
│   ├── config/
│   │   └── GameParameters.ts       # ALL tunable parameters in one place (470 lines)
│   ├── terrain/
│   │   ├── TerrainGenerator.ts     # Procedural voxel terrain mesh + shader injection (639 lines)
│   │   ├── GridSystem.ts           # 4000-unit grid with observer pattern (131 lines)
│   │   └── LightingSystem.ts       # Sun, sky, halo, day/night cycle (569 lines)
│   ├── ui/
│   │   └── ReflectionControls.ts   # In-game sliders for terrain/sun params (167 lines)
│   ├── debug/
...
- Terrain height: 0–800 units (controlled by HEIGHT_SCALE in GameParameters)
...
### Phase 2 — Terrain Improvement (Next Priority)
**Goal**: More dramatic, readable terrain — tall mountains, deep valleys, gradual slopes. No jagged spikes.

**Key parameters in `GameParameters.ts` (TerrainParameters section):**
- `HEIGHT_SCALE: 800` — increase to ~1800–2500 for taller mountains
- `PERSISTENCE: 0.65` — controls how much each octave contributes; lower (e.g. 0.45–0.55) = smoother, more gradual slopes
- `LACUNARITY: 1.6` — frequency multiplier per octave; keep around 1.6–1.8
- `NOISE_SCALE: 0.001` — lower = larger features; try 0.0006–0.0008 to widen mountains and valleys
- `BASE_NOISE_OCTAVES: 6` / `NOISE_OCTAVES: 8` — keep as-is
- `BASE_AMPLITUDE_FALLOFF: 0.7` — controls base terrain flatness; increase slightly for wider valleys

**Approach:**
- Increase amplitude (HEIGHT_SCALE) as the primary lever
- Reduce PERSISTENCE to keep slopes gradual (prevents sharp jagged ridges at high amplitude)
- Reduce NOISE_SCALE to widen features so valleys feel like valleys not pits
- Optionally add a valley carving pass: a low-frequency noise layer subtracted from height to cut a main river valley through the map

**Definition of done:** Running terrain has at least one clearly readable valley/river corridor, mountain peaks feel tall and dramatic, slopes are walkable-looking (not cliffs everywhere).

### Phase 3 — Add Units (was Phase 2)
