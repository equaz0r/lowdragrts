...
- ✅ **Sun glitter path — real Blinn-Phong specular against a noisy normal, not a tracked point** (`TerrainMaterial.ts`, 11 Aug 2026, second rewrite) — the "fake screen-space tracking" attempt (previous bullet history) shipped typechecked/built but was never actually the right target: Simon's follow-up hand-annotated screenshot showed he expected a WIDE WEDGE (roughly the sun's width near the horizon, narrowing to a point near the camera) — the standard real "sun glitter path" seen on wavy water (Cox-Munk wave-slope-distribution optics), not a single tracked dot. No amount of aligning ONE point (neither the original physically-correct `reflect()` against a smooth normal, nor the fake screen-space hack) could ever produce that shape — both are fundamentally single-point mechanisms. Full analysis + verdict (not a camera/grid/skybox/engine issue, doesn't need raycasting or SSR, achievable in the existing `onBeforeCompile` architecture) is in the plan file referenced in `## Current Status`. Rewrote `calculateReflection()` again: removed the screen-space hack entirely; added `calculateSunGlitter()`, a small self-contained GLSL function (world position, normal, sun direction, view direction, `time` only — deliberately generic, not terrain-specific, so it can be reused for buildings/units later per Simon's forward-compat note) that perturbs the normal with fine animated hash noise (`GLITTER_FREQUENCY`/`SPEED`/`AMPLITUDE` — local constants, same pattern as the sea-shimmer constants, NOT gated to low/flat ground like the sea shimmer is) and runs a real Blinn-Phong half-vector specular test against it, gated by facing-the-sun and slope (steep cliffs stay solid rock, not sparkly). The wedge shape is meant to emerge from this geometry on its own — deliberately NOT hand-masked, since a mask would just be a third version of the same failed single-point hack. `ReflectionParameters.SUN_GLITTER_WEIGHT` (1.1) is the only new tunable in the weighted-sum config; noise shape constants live in `TerrainMaterial.ts`. First-pass numbers, not yet visually confirmed — check this first next session.
...
- **Sun glitter is real per-fragment Blinn-Phong specular against a noisy normal now** (see What's Working above) — `calculateSunGlitter()`, generic/reusable, not terrain-specific. If a reflection complaint comes in, check which system it actually is first: wrong SHAPE/SPREAD (too pinpoint, too wide, wrong side of the sun) → this glitter code (`GLITTER_FREQUENCY`/`SPEED`/`AMPLITUDE`/`SHININESS` constants + `SUN_GLITTER_WEIGHT`); too bright/dim/rare in general → the weighted-sum/clamp system below, which is unchanged and separate.
- Two failed prior approaches, don't repeat either: (1) `reflect()`/N·L against the plain SMOOTH geometric normal — geometrically correct but produces one precise point wherever local slope satisfies the mirror condition, essentially never "under the sun" visually; (2) comparing camera→fragment vs camera→sun screen-space direction — can only ever produce a small aligned blob, never a spread. Both are single-point mechanisms; only a noisy-normal + real specular combination produces the expected wide, camera-converging wedge (real optics — Cox-Munk wave-slope-distribution theory — not something to hand-mask).
- `GLITTER_AMPLITUDE`/`GLITTER_SHININESS` are the main levers if the wedge doesn't spread/narrow convincingly: too little noise amplitude collapses back toward a tiny dot; too much turns the whole terrain into diffuse sparkle mush with no wedge definition. `SUN_GLITTER_WEIGHT` (1.1, in `ReflectionParameters`) is the overall contribution to the weighted sum. All first-pass, untested numbers.
- **First-pass amplitude (0.35) was too small to ever be visible** — traced why: `jitteredNormal = normalize(geomNormal + tilt)` starts from a mostly-VERTICAL `geomNormal` on flat/rolling ground, and an additive XZ-only tilt can only rotate it ~atan(amplitude) degrees off vertical (19° at 0.35). But for a grazing view angle against a near-horizon sun (exactly Simon's screenshot: Sun Height near its -0.8 minimum, low camera), the Blinn-Phong half-vector `H=normalize(sunDir+viewDir)` is itself close to HORIZONTAL — nowhere near reachable at that tilt range, so the specular test was failing almost everywhere, which read as "nothing changed" even with a confirmed-fresh bundle. Raised to `GLITTER_AMPLITUDE=2.2`/`GLITTER_SHININESS=10.0` (from 0.35/36) to make the effect unmissable first — deliberately erring toward "too much" over "too subtle" given there's no live preview to fine-tune against; expect to narrow the sharpness back up once the general area/shape is confirmed visible.
