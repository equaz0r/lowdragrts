...
## Known Issues
- 🟠 **Sun-behind-camera glint fix is implemented but still needs live confirmation.** The failed live result had a concrete coordinate-space bug that the standalone simulator could not reproduce: `TerrainMaterial.ts` named `normalMatrix * normal` as `vWorldNormal`, but Three.js's `normalMatrix` produces a **view-space** normal, which was then compared with world-space sun/camera/fragment vectors. The shader now converts it back with Three's `inverseTransformDirection(viewSpaceNormal, viewMatrix)` before any reflection dot products. TypeScript, Webpack and all forward/back-glint simulation checks pass. Remaining step: live-test `Debug: Glitter Only` while orbiting from sun-in-front to sun-behind; do not retune constants until that coordinate-space fix is visually verified.
...
- Render mesh divisions = grid cell count (125), deliberately equal — 1 mesh quad = 1 grid cell, so `TerrainGrid.ts`'s lines sit on real mesh vertices with zero gap.
...
- `SUN_TERRAIN_LIGHT_SCALE` decouples the sun's visual brightness (disc/halo) from how much light it casts on terrain. The "Sun Intensity" slider's effect on terrain lighting is dead code in practice — only elevation drives terrain light intensity.
