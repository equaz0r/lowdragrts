...
- ✅ **"Sea" shimmer** (`TerrainMaterial.ts`) — low/flat ground (bottom ~10-15% of the height range) gets a cheap time-varying normal perturbation for the reflection calculation only (not the real geometric normal), so glints dance like light on water even though the mesh never moves. Not real waves — see `## Water / Sea — future options` below for what a fuller version would take and why it wasn't done now.
