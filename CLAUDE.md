...
- ✅ **Live sun direction in the reflection shader** — `sunDirection` used to be set ONCE at material creation (`new Vector3(-1, 0.3, 0)`, a rough guess) and never updated, so the reflection glint never actually tracked the real orbiting sun — any apparent alignment was coincidence. Now pushed from `LightingSystem.getSunDirection()` every frame in `TerrainGenerator.update()`, same pattern as `sunColor`.
