...
## Known Issues
- 🔴 **Sun reflection glint doesn't correctly appear when the sun is behind the camera.** Real specular reflection only needs the surface normal to bisect sun direction and view direction — it doesn't require the sun to be in front of the camera. `calculateSunGlitter()`'s wedge has a hard `inFront` gate that zeros the effect whenever the camera looks away from the sun (by design, to avoid an earlier single-point/streak bug — not emergent physical rarity). A second term, `calculateBackGlint()` (real half-vector specular against the actual terrain normal), was added to cover this case and passed all shape checks in `tools/simulate-sun-glitter.js` — **but live-testing in-browser showed no visible fix.** Not yet re-diagnosed. Next steps: use the debug isolation view (`debugGlitterValue` already includes `backGlint`) against real terrain live instead of more simulation-only guessing; the simulation only tested synthetic sine-wave slopes, not the actual generated heightfield — that gap is the prime suspect.
